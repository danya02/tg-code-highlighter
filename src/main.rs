...
async fn main() -> anyhow::Result<()> {
    ...
    let config = Config{ null_chat_id: -992674722 }; // TODO: accept this from outside 
    let mut state = State{api, pool, font_system, swash_cache, config, syntax_set, theme_set};
    ...
    while let Some(update) = stream.next().await {
        ...
        match update.kind {
            ...
            UpdateKind::InlineQuery(query) => {
                process_inline_query(&mut state, query).await?;
            }
            UpdateKind::ChosenInlineResult(chosen_inline_result) => {
                ...
                query!("UPDATE gist SET is_ephemeral=0 WHERE id=?", chosen_inline_result.result_id).execute(&state.pool).await?;

                // Also remove the expired ephemeral gists
                let delete_cutoff = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs() as i64 - 1800; // 30 minutes
                query!("DELETE FROM gist WHERE is_ephemeral>0 AND sent_at_unix_time<?", delete_cutoff).execute(&state.pool).await?;
            }
            _ => {}
        }
    }
...
async fn process_inline_query(
    ...
    let upload = api.send(SendPhoto::new(ChatId::from(config.null_chat_id), photo_upload)).await?;
    let file_id = if let MessageKind::Photo { data, .. } = upload.kind {
        let first = data[0].clone();
        let largest = data.iter().fold(first, |acc, item| if (acc.width, acc.height)<(item.width, item.height) {item.clone()} else {acc});
        largest.file_id
    } else {
        return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, format!("Uploaded image with photo but it wasn't a photo message?!")))?;
    };
